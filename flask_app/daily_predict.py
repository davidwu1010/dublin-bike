...
def bike_predict_daily(number_input):
    ...
    daily_df = combined_df.resample('D', on='scraping_time').mean( ).reset_index( )
    ...
    bike_daily = pd.DataFrame({'weekday': weekday, 'bike_daily_predict': y_prediction * total_bike_stand})
    days = {1: 'Mon', 2: 'Tue', 3: 'Wed', 4: 'Thu', 5: 'Fri', 6: 'Sat', 7: 'Sun'}
    bike_daily['weekday'] = bike_daily['weekday'].apply(lambda x: days[x])
    bike_daily = bike_daily.to_json(orient='records')
