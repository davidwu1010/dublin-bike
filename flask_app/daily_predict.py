...
def bike_predict_daily(number_input):
    ...
    bike_df = pd.read_sql_query(sql=query_bike, con=engine,
                                params={'number': number_input},
                                parse_dates=['scraping_time'])
    weather_df = pd.read_sql_query(sql=query_weather, con=engine,
                                   params={'number': number_input},
                                   parse_dates=['scraping_time'])
    ...
    def data_cleaning(df):
        ...
        col_to_drop = ['address', 'site_names', 'bonus', 'last_update',
                       'datetime', 'icon', 'lon', 'lat', 'stationNum',
                       'available_bike_stand', 'description', 'status',
                       'sunset']
    ...
    daily_df = combined_df.resample('H', on='scraping_time').mean()\
        .reset_index()
    ...
    data = pd.DataFrame({'number': daily_df['number'], 'hour': hour,
                         'weekday': weekday,
